...
public class removeadminCommand extends SubCommand {
    @Override
    ...
    public String getSyntax() {
        return "/porman removeadmin <傳送門名字> <玩家名字>";
    }
    ...
    public void perform(CommandSender commandSender, String[] args) {
        ...
        if (!portal.getAllAdmin().contains(target.getUniqueId().toString())) {
            p.sendMessage(prefix + " 這個玩家本就不是管理員。");
            return;
        }
        ...
        portal.removeAdmin(target.getUniqueId());
        updatePortal(portal);
        p.sendMessage(prefix + " 成功從管理員中移除 " + ChatColor.GREEN + target.getName() + ChatColor.WHITE + " 。");
